...
class UserBookHomeService: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_userbookhomeservice)
...
//            Toast.makeText(this@MainActivity, msg, Toast.LENGTH_SHORT).show()
        ...
        btn_book.setOnClickListener{
            ...
            var app_address = address.text.toString().trim()
            ...
            if(app_fullname.isEmpty() || app_email.isEmpty() || app_phone.isEmpty() || app_cnic.isEmpty() || app_address.isEmpty() || app_date.equals("")){
            ...
            else if(!app_address.matches(Regex(".*$"))){
                ...
                databaseReference.child("saloonify").addValueEventListener(object : ValueEventListener {
                    override fun onDataChange(dataSnapshot: DataSnapshot){
                        databaseReference.child("home").child(app_cnic).child("fullname").setValue(app_fullname)
                        databaseReference.child("home").child(app_cnic).child("email").setValue(app_email)
                        databaseReference.child("home").child(app_cnic).child("contact").setValue(app_phone)
                        databaseReference.child("home").child(app_cnic).child("cnic").setValue(app_cnic)
                        databaseReference.child("home").child(app_cnic).child("address").setValue(app_address)
                        databaseReference.child("home").child(app_cnic).child("stylist").setValue(app_stylist)
                        databaseReference.child("home").child(app_cnic).child("date").setValue(app_date)
                        databaseReference.child("home").child(app_cnic).child("category").setValue(app_category)
                        databaseReference.child("home").child(app_cnic).child("time").setValue(app_time)


                        Toast.makeText(applicationContext,"Request pending", Toast.LENGTH_SHORT).show()
